...
|FT50-43 or FT50-77 (~12mm OD, type 43/77)
|10
|Amazon
|$10.00

|Enameled magnet wire
|30 AWG, 1 oz spool (BNTECHGO or similar)
|1
|Amazon
|$8.00

|*Comparator*
|*LM393, DIP-8* _(not LM358 — see note)_
|4
|Amazon
|$4.00

|NPN transistor
|2N2222, TO-92 package
|20
|Amazon
|$3.00
...
[WARNING]
====
*Ferrite core material matters.* Use type 43 or type 77 MnZn/NiZn
...
|Wire stripper / tweezers
|For magnet wire work
|~$5
|===
...
. Connect the preamp Collector to the *(+) non-inverting input* of one
  LM393 channel.
. Build the reference divider (33kΩ / 1.5MΩ from 5V to GND) and connect
  the midpoint to the *(-) inverting input*. Add 100nF from (-) to GND.
. The LM393 output is *open-collector* — add a *10kΩ pull-up* to +5V.
. Connect the output to an Arduino interrupt pin (D2 or D3) or digital input.
...
|All bits read as 1
|Word wire coupling into all sense lines
|Route word wire away from sense leads; use twisted sense pair

|Weak/no spike on scope (<50 mV)
|Wrong ferrite material
|Confirm cores are type 43 or 77 — not iron-powder or type 61

|Spike present but comparator flat
|LM393 threshold too high, or bad wiring
|Lower reference divider; verify LM393 pinout and pull-up

|Intermittent reads on D4–D9
|Polled read missing short pulse
|Increase PULSE_US; or add 74HC74 latch on comparator output

|Correct pattern, wrong bit order
|Bit index inverted in firmware
|Swap `(1 << (7-i))` to `(1 << i)` in the polling loop

|Random false 1s
|Breadboard noise / capacitive coupling
|Add 100nF decoupling on LM393 Vcc; twist all sense leads

|Transistor not switching
|Wrong 2N2222 pinout
|Pinout varies by brand — verify EBC order on your datasheet
|===
